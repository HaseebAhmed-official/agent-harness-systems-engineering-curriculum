...
- Checkpoint date: 2026-09-07. Use Git for current publication state; last entering pushed checkpoint was `5dc8b76`.
...
Semester 1 lessons 1-16 have expanded instructional guidance; all 10 printed examples passed on Windows at `5dc8b76`. Semester 2 lessons 1-4 now add requirements/SLO arithmetic, orchestration/failure comparison, actual process-exit recovery, and stale memory-publication reasoning. All four exact Python examples passed in the current worktree on Windows Python 3.13.1 and WSL Ubuntu Python 3.13.9. A trusted child exits abruptly with code 23 after committing its lease and a local synthetic effect. Blind retry produces two effects; the single-task receipt-reconciliation control preserves one, although both local tasks report success. This is actual process-exit evidence, not power-loss resilience, a real transactional external service, or complete LAB-C2. The memory race remains a sequential in-memory schedule, not concurrent durable deletion proof. Weeks 5-16 of Semester 2 and full lab/exam/learner validation remain pending.
...
The preceding goal turn was progress: it published Semester 1 instruction at `5dc8b76`. This continuation expanded Semester 2 lessons 1-4, checked Google SRE, Anthropic, and AWS primary guidance, and executed all new examples on Windows and WSL. Ubuntu's default Python 3.10.12 is below the declared >=3.11 requirement; the existing `/root/.local/share/uv/python/cpython-3.13.9-linux-x86_64-gnu/bin/python3.13` was used without installation changes. No new files or runtime source changes were needed. No live process is pending. Older session-log Next cells are historical.

## Next Actions

1. Expand Semester 2 lessons 5-8 next: MCP, A2A, threat modeling, and defense in depth. Inspect the pinned protocol/authorization implementations and their lab contracts; verify current primary specifications before teaching protocol-era details. Continue with lessons 9-16 afterward. Reuse the guide and existing evidence records; do not duplicate the four executed examples or repeat completed corpus/source discovery. All teaching/lab learner-validation gates remain open.
