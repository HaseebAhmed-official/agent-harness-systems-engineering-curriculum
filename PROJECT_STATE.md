...
2026-09-07 teaching milestone: Semester 1 lessons 1-16 now have instructional guidance, worked reasoning, practice, remediation, and evidence boundaries. All 10 exact Python blocks passed on Windows Python 3.13.1. Additional negative controls verified raw fake-marker disclosure in failure events/stored tool messages and output-only grading that misses duplicate synthetic effects. New lessons distinguish approved arguments from authenticated authority, relevance from trustworthy memory, and configured evaluation acceptance from release justification. These are bounded teaching observations, not runtime repairs. Semester 2 remains outline-level; full lab/exam execution, learner outcomes, accessibility usability, assessor reliability, and independent reproduction remain incomplete or unmeasured.
